...
class AcademicRouter:
    ...
    def parse_student_query(raw_prompt: str, gui_model: str = "google/gemini-3.8-flash", gui_subject: str = "") -> StudyIntent:
        ...
        # Check multi-engine combinations first (e.g. "using perpelixity and claude")
        multi_engine_match = re.search(
            r"\b(using|use|from|with|on)?\s*(perpelixity|perplexity|perpelexcity|perpelxity|perplexcity)\s*(and|\&|\+)\s*(claude|clud|claud|cloude)\b|"
            r"\b(using|use|from|with|on)?\s*(claude|clud|claud|cloude)\s*(and|\&|\+)\s*(perpelixity|perplexity|perpelexcity|perpelxity|perplexcity)\b|"
            r"\b(using|use|from|with|on)?\s*(all\s*engines|all\s*web|combined\s*engines|both\s*engines|both\s*web|multi\s*engine)\b",
            prompt_lower
        )

        if multi_engine_match:
            use_gui = True
            gui_target = "all"
            clean_text = re.sub(
                r"\b(using|use|from|with|on)?\s*(perpelixity|perplexity|perpelexcity|perpelxity|perplexcity)\s*(and|\&|\+)\s*(claude|clud|claud|cloude)\b|"
                r"\b(using|use|from|with|on)?\s*(claude|clud|claud|cloude)\s*(and|\&|\+)\s*(perpelixity|perplexity|perpelexcity|perpelxity|perplexcity)\b|"
                r"\b(use\s+)?(all\s+engines|all\s+web|combined\s+engines|both\s+engines)\b",
                "",
                clean_text,
                flags=re.IGNORECASE
            ).strip()
        elif re.search(r"\b(using|use|from|with|on)?\s*(claude|clud|claud|cloude)(\s+ai)?(\s+web)?\b", prompt_lower):
            use_gui = True
            gui_target = "claude"
            clean_text = re.sub(r"\b(and\s+)?(take|get|fetch)\s+(this\s+)?content\s+from\s+(claude|clud|claud|cloude)(\s+ai)?:?\s*", "", clean_text, flags=re.IGNORECASE).strip()
            clean_text = re.sub(r"\b(using|use|from|with|on)\s+(claude|clud|claud|cloude)(\s+ai)?(\s+web)?(\s+to)?\b", "", clean_text, flags=re.IGNORECASE).strip()
            clean_text = re.sub(r"\b(claude|clud|claud|cloude)(\s+ai)?(\s+web)?\b", "", clean_text, flags=re.IGNORECASE).strip()
        elif re.search(r"\b(using|use|from|with|on)?\s*(gemini|gemeni|gemni|jimini)(\s+ai)?(\s+web)?\b", prompt_lower):
            use_gui = True
            gui_target = "gemini"
            clean_text = re.sub(r"\b(and\s+)?(take|get|fetch)\s+(this\s+)?content\s+from\s+(gemini|gemeni|gemni|jimini)(\s+ai)?:?\s*", "", clean_text, flags=re.IGNORECASE).strip()
            clean_text = re.sub(r"\b(using|use|from|with|on)\s+(gemini|gemeni|gemni|jimini)(\s+ai)?(\s+web)?(\s+to)?\b", "", clean_text, flags=re.IGNORECASE).strip()
            clean_text = re.sub(r"\b(gemini|gemeni|gemni|jimini)(\s+ai)?(\s+web)?\b", "", clean_text, flags=re.IGNORECASE).strip()
        elif re.search(r"\b(using|use|from|with|on)?\s*(perpelixity|perplexity|perpelexcity|perpelxity|perplexcity)(\s+ai)?(\s+web)?\b", prompt_lower):
            use_gui = True
            gui_target = "perplexity"
            clean_text = re.sub(r"\b(and\s+)?(take|get|fetch)\s+(this\s+)?content\s+from\s+(perpelixity|perplexity|perpelexcity|perpelxity):?\s*", "", clean_text, flags=re.IGNORECASE).strip()
            clean_text = re.sub(r"\b(using|use|from|with|on)\s+(perpelixity|perplexity|perpelexcity|perpelxity)(\s+ai)?(\s+web)?(\s+to)?\b", "", clean_text, flags=re.IGNORECASE).strip()
            clean_text = re.sub(r"\b(perpelixity|perplexity|perpelexcity|perpelxity)(\s+ai)?(\s+web)?\b", "", clean_text, flags=re.IGNORECASE).strip()
        ...
        clean_topic = re.sub(r"\b(using|use|from|with|on)?\s*(gemini|gemeni|gemni|jimini|claude|clud|claud|cloude|perplexity|perpelixity|perpelexcity|perpelxity|perplexcity)\b", "", clean_topic, flags=re.IGNORECASE).strip()
